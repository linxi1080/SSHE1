					...
					for (FileItem item : items) {
						if (item.isFormField()) {// 是文本域
							if (item.getFieldName().equals("name")) {
								tempFileName = item.getString("UTF-8");
...
//								String chunkName = tempFileName;
								//new begin
								String chunkName = UUID.randomUUID().toString().replace("-", "").concat(".").concat(FilenameUtils.getExtension(tempFileName));;
								newFileName = chunkName;
								//new end
								if (chunk != null) {
...
//					newFileName = UUID.randomUUID().toString().replace("-", "").concat(".").concat(FilenameUtils.getExtension(tempFileName));
					if (chunks == null) {// 如果不分块上传，那么只有一个名称，就是临时文件的名称
						//new begin
						if (newFileName.isEmpty() || newFileName.equals("") || newFileName==null){
